<?php
/**
 * Policies & Consent tab: intro paragraph, status summary, and the Save form (Policy Version/Effective Date,
 * Transaction Confirmation, and one section per CHADA_TRAVEL_Policy_Registry::get_definitions() entry). See
 * CHADA_TRAVEL_Admin_Settings::render_policies_consent_tab().
 *
 ...
 * @var string $chada_travel_policy_bundle_fields_html Pre-escaped.
 * @var string $chada_travel_transaction_confirmation_html Pre-escaped.
 * @var string $chada_travel_sections_html Pre-escaped, one policy-section.php per definition.
 */
...
assert(isset($chada_travel_policy_bundle_fields_html));
assert(isset($chada_travel_transaction_confirmation_html));
...
<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- pre-escaped by the caller, see the _html naming convention documented in CHADA_TRAVEL_Template. ?>
<?php echo $chada_travel_policy_bundle_fields_html; ?>
<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- pre-escaped by the caller, see the _html naming convention documented in CHADA_TRAVEL_Template. ?>
<?php echo $chada_travel_transaction_confirmation_html; ?>
<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- pre-escaped by the caller, see the _html naming convention documented in CHADA_TRAVEL_Template. ?>
<?php echo $chada_travel_sections_html; ?>

<p class="submit"><button type="submit" class="button button-primary">
<?php esc_html_e('Save Policies & Consent', 'chada-travel'); ?>
</button></p>
</form>
